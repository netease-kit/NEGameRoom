...
public class SudFSMMGCache {
  ...
  private final HashMap<String, SudMGPMGState.MGCommonPlayerPlaying> playerPlayingMap =
      new HashMap<>(); // 记录玩家的游戏状态

  // 队长状态 处理
  public void onPlayerMGCommonPlayerCaptain(
      String userId, SudMGPMGState.MGCommonPlayerCaptain model) {
    if (model != null) {
      if (model.isCaptain) {
  ...
  // 玩家游戏状态
  public void onPlayerMGCommonPlayerPlaying(
      String userId, SudMGPMGState.MGCommonPlayerPlaying model) {
    if (model != null) {
