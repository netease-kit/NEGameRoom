...
public class GameCommonStateUtils {
   ...
   * @return
   */
  public static String parseMGCommonPublicMessage(
      SudMGPMGState.MGCommonPublicMessage publicMessage, String languageCode) {
    if (publicMessage == null || publicMessage.msg == null || publicMessage.msg.size() == 0) {
      return null;
    }
    StringBuilder sb = new StringBuilder();
    for (SudMGPMGState.MGCommonPublicMessage.MGCommonPublicMessageMsg msgModel :
        publicMessage.msg) {
   ...
   * @return 返回选择的字符串
   */
  public static String parseI18nText(
      String languageCode, SudMGPMGState.MGCommonPublicMessage.MGCommonPublicMessageMsgText model) {
  ...
  /** 精准匹配 */
  private static String i18nPrecise(
      String languageCode, SudMGPMGState.MGCommonPublicMessage.MGCommonPublicMessageMsgText model) {
    switch (languageCode) {
      case "zh-CN":
        return model.zh_CN;
  ...
  private static boolean isMatchLanguage(String languageCode, String matchLanguageCode) {
    if (languageCode != null && matchLanguageCode != null) {
      return matchLanguageCode.equals(languageCode)
          || languageCode.startsWith(matchLanguageCode + "-");
    }
    return false;
  }
}
