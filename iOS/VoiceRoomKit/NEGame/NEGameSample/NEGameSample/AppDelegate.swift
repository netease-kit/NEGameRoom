...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
  ...
  func baseInit(_ callback:((Bool)->Void)? = nil){
    ...
    loginSampleExtras["baseUrl"] = Configs.nemoBaseUrl
...
extension AppDelegate {
  ...
  func initAllModules(_ appKey: String, extra: [String: String] = [String: String](), callback: @escaping (Int, String?) -> Void) {
    ...
    initGame(appKey, extras: extra) { code, msg, _ in
      if code != 0 {
        callback(code, msg)
      } else {
        initQueue.async {
  ...
  func initGame(_ appKey:String ,extras: [String: String] = [String: String](), callback: @escaping (Int, String?, Any?) -> Void) {
    let config = NEVoiceRoomKitConfig()
    config.appKey = appKey
    var voiceRoomExtras = extras
    voiceRoomExtras["baseUrl"] = Configs.nemoBaseUrl
    config.extras = voiceRoomExtras
    NEGameUIManager.instance.initialize(config: config, configId: Configs.gameConfigId, callback: callback)
    listenGameEvent()
  }
}
...
extension AppDelegate {
  ...
  public func loginRoom(shouldInit: Bool = false , callback:((Bool)->Void)? = nil ) {
    ...
    func login() {
      NEGameUIManager.instance.login(account: userUuid, token: userToken, nickname: userName) { code, msg, _ in
        if code == 0 {
          DispatchQueue.main.async {
            // 刷新头像与昵称
...
extension AppDelegate {
  func listenGameEvent() {
    NEGameUIManager.instance.gameClientEvent = { event in
      switch event {
      case .kickOut,.forbidden:
