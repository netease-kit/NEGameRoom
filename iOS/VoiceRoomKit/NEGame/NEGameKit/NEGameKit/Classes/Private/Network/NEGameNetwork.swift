...
let NE = NEGameNetwork.shared

/// 网络请求类
internal class NEGameNetwork {
  let tag: String = "NEGameNetwork"
  static let shared = NEGameNetwork()
  // 配置信息
  ...
  func request(_ url: String,
    ...
    NEGameLog.networkLog(
      tag,
      desc: "HTTPRequest\nUrl: \(url).\nMethod: \(method)\nHeaders: \(headers)\nBody: \(body?.prettyJSON ?? "")"
    )
    guard let URL = URL(string: url) else {
      NEGameLog.errorLog(tag, desc: "\(url). Bad Url.")
      failed(makeError(NEGameErrorCode.failed, "Bad url"))
      return
    }
    ...
    let dataTask = session.dataTask(with: request) { [weak self] data, response, error in
      ...
      guard httpResp.statusCode == 200 else {
        ...
        failed(makeError(NEGameErrorCode.failed,
                         "Http error with status: \(httpResp.statusCode)"))
        return
      }
      ...
      guard let response = try? JSONSerialization.jsonObject(
        ...
        failed(makeError(NEGameErrorCode.failed, "Cannot decode content data"))
        return
      }
      // 打印requestId
      if let requestId = response["requestId"] as? String {
        NEGameLog.infoLog(self.tag, desc: "Url:\(url) RequestId:\(requestId)")
      }
      guard let code = response["code"] as? Int else {
        failed(makeError(NEGameErrorCode.failed, "Empty code in response body!"))
        return
      }
      guard code == 200 else {
      ...
      NEGameLog.infoLog(self.tag, desc: "Response data:\n\(response.prettyJSON)")
      success(dic)
    }
    dataTask.resume()
  }
  ...
  func request<T>(_ url: String,
    ...
    request(url, method: method, headers: headers, body: body) { dic in
      ...
      success(NEGameDecoder.decode(returnType, param: dic) ?? nil)
    } failed: { error in
      failed(error)
    }
  }
